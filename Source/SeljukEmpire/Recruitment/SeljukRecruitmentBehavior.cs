...
namespace SeljukEmpire.Recruitment
{
    ...
    public class SeljukRecruitmentBehavior : CampaignBehaviorBase
    {
        ...
        private static bool IsSeljukSettlement(Settlement settlement)
        {
            ...
            // Fast-path prefix check for our own core settlements (avoids a Clan/Kingdom lookup on
            // the hot per-notable loop below); the Kingdom check afterwards is the authoritative one
            // and already covers every Seljuk-owned settlement, including future territory changes.
